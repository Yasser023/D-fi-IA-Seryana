...
df = pd.read_csv('df_seryana.csv')
df_final = df.join(hotels, on=['hotel_id', 'city'])
dataset = df_final.copy()
dataset = dataset.drop(['Unnamed: 0.1','Unnamed: 0','hotel_id','avatar_id'], axis=1)
...
correspondance_language = pd.DataFrame({'language':df_encoder['language'].unique(),'encoding':df_encoder['language_encoded'].unique()})
correspondance_group = pd.DataFrame({'group':df_encoder['group'].unique(),'encoding':df_encoder['group_encoded'].unique()})
correspondance_brand = pd.DataFrame({'brand':df_encoder['brand'].unique(),'encoding':df_encoder['brand_encoded'].unique()})
